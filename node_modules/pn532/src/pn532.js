...
class PN532 extends EventEmitter {
    ...
    */
    constructor(hal, options) {
        ...
        this.eventScanTag= new EventEmitter();
        ...
        this.on('newListener', (event) => {
            ...
            if(this.pollInterval > 0){
                if (event === 'tag') {
                    console.log('poll',this.pollInterval);
    ...
    InRelease(Tg=1){
        let commandBuffer=[c.COMMAND_IN_RELEASE,Tg];
    ...
    InListPassiveTarget(maxTg=1){
        if(maxTg>2){
            console.log('Not support number of scanning tag greater than 2');
            return -1;
        } 
        var maxNumberOfTargets = maxTg;
    ...
    ScanTagWithEvent(){
        this.InListPassiveTarget()
        .then((tag_info)=>{
            this.eventScanTag.emit('tag', tag_info);
        })
        .catch((err)=>{
            this.eventScanTag.emit('tag_err', err);
        })
    }
    DestroyScanTagEvent(){
        eventScanTag.removeListener('tag',()=>{console.log('removed tag event')});
        eventScanTag.removeListener('tag_err',()=>{console.log('removed tag_err event')});
    }
